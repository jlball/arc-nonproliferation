import numpy as np
import openmc
from arc-nonproliferation.components import *
...
class Device(openmc.model.Model):
    ...
    def add_tally(self, name, scores, nuclides=None, filters=None):
        ...
        # Allows nuclides to be set to 'all' which isn't particularly useful I don't think
        if nuclides is not None:
            if nuclides in ['all', ['all']]:
                nuclides = list(set([nuc for cell in cells 
                                     for nuc in cell.fill.get_nuclides()]))
